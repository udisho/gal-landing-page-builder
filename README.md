...
A browser-only tool that turns a plain landing page (HTML) into a styled Gal Travels landing page, ready to paste into ravpage.
...
2. Paste the full HTML of an existing landing page into the textarea.
...
- Hero image (from `og:image` or the first `<img>`)
...
The local server adds a `/api/fetch?url=` proxy so you can fetch remote pages via URL during development. The published (GitHub Pages) version is static — paste-only.
